---
id: qaas.userinterfaces.runner.commands.template
type: reference
status: stable
since: 2.0.0
...
Usage:
 dotnet run [Dotnet Parameters] -- [Command] [Values] [Flags]
  ...
  --preserve-reference-names         (Default: false) Use with -p or --push-references. By default, these options prefix
                                     the unique name field of each item added from a reference file with the replace
                                     keyword. This flag skips that prefix and keeps the raw name from the reference
                                     file.

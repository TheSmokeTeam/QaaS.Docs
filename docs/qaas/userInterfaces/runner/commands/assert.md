---
id: qaas.userinterfaces.runner.commands.assert
type: reference
status: stable
since: 2.0.0
last_verified: 2026-05-23
applies_to: [runner]
keywords: [qaas, userinterfaces, runner, commands, assert]
summary: "Run a qaas test with only the assertions on already existing sessionData."
---
...
> TL;DR — Run a qaas test with only the assertions on already existing sessionData.

## Invocation {: #invocation}

```bash
dotnet run <dotnet-parameters> -- assert <config-file> [flags]
```
...
### Assert against the stored session data for a configuration {: #assert-against-the-stored-session-data-for-a-configuration}

```bash
dotnet run -- assert test.qaas.yaml
```

### Assert and open the results folder {: #assert-and-open-the-results-folder}

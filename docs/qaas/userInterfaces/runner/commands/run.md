---
id: qaas.userinterfaces.runner.commands.run
type: reference
status: stable
since: 2.0.0
last_verified: 2026-05-23
applies_to: [runner]
keywords: [qaas, userinterfaces, runner, commands, run]
summary: "Run a qaas test according to the given configurations."
---
...
> TL;DR — Run a qaas test according to the given configurations.
...
| Results | `-e`, `--empty-results-directory` | Yes | No | False | `bool` | If flag is enabled will automatically empty the results directory before running. |
...
| General | `-o`, `--open-reportportal` | Yes | No | False | `bool` | Opens each successfully published ReportPortal report in the default browser. ReportPortal reporting must be enabled in the configuration. |
...
| Results | `-s`, `--serve-results` | Yes | No |  | `string` | Serves Allure output after performing the assertions.<br />If the flag is provided without a value it serves the default raw results folder 'allure-results'.<br />Provide a folder name such as 'allure-report' to open a generated report directory, which is useful for Allure 3 flows. |
...
### Run the default configuration file {: #run-the-default-configuration-file}

```bash
dotnet run -- run test.qaas.yaml
```

### Run only one case and one session {: #run-only-one-case-and-one-session}

```bash
dotnet run -- run test.qaas.yaml -c cases -n happy-path -i Checkout
```

### Run and open an Allure 3 style report folder {: #run-and-open-an-allure-3-style-report-folder}
...
Usage:
 dotnet run [Dotnet Parameters] -- [Command] [Values] [Flags]
  ...
  -s folder, --serve-results=folder    Serves Allure output after performing the assertions.
  ...
  -o, --open-reportportal              (Default: false) Opens each successfully published ReportPortal report in the
                                       default browser. ReportPortal reporting must be enabled in the configuration.

  -e, --empty-results-directory        (Default: false) If flag is enabled will automatically empty the results
  ...
  --preserve-reference-names           (Default: false) Use with -p or --push-references. By default, these options
                                       prefix the unique name field of each item added from a reference file with the
                                       replace keyword. This flag skips that prefix and keeps the raw name from the
                                       reference file.

  -c, --cases                          Name of the folder containing all the different .yaml case files that cases are
                                       ran with, If no folder is given will run the .qaas.yaml file as is without cases.
  ...
  --resolve-cases-last                 (Default: false) When this flag is used cases will be resolved after all other
                                       types of configuration resolutions, instead of its default behaviour which is
                                       after overwrite files and before references.
...
- [Publishers YAML view](../configurationSections/sessions/types/publishers-yamlView.md)
- [PublisherBuilder API](../../../functions/builders/publishers.md)
